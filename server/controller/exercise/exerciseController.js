...
const exerciseController = {
  ...
  deleteExercise: async (req, res) => {
    try {
      ...
      if (exercise == null)
        return res.status(404).json({ message: `Exercise ${req.params.id} not found` });
  ...
  getExerciseSubmissions: async (req, res) => {
    const exerciseId = req.params.id;
  ...
  /* 
    ...
    */
  mergeCustomTests: async (req, res) => {
    const exerciseId = req.params.id;
    try {
      ...
      if (exercise.testCases.length >= 30)
        return res.status(406).json({
          message: 'The exercise already has at least 30 test cases!',
        });
  ...
  getExerciseReports: async (req, res) => {
    const exerciseId = req.params.id;
    try {
  ...
  postExerciseReport: async (req, res) => {
    const exerciseId = req.params.id;
  ...
  /*
    ...
    */
  toggleLikeExercise: async (req, res) => {
    const exerciseId = req.params.id;
  ...
  getExerciseShowcases: async (req, res) => {
    const exerciseId = req.params.id;
  ...
  postExerciseShowcase: async (req, res) => {
    const exerciseId = req.params.id;
  ...
  getExerciseComments: async (req, res) => {
    const exerciseId = req.params.id;
  ...
  postExerciseComment: async (req, res) => {
    const exerciseId = req.params.id;
    const { text } = req.body;
    ...
    try {
      ...
      if (err instanceof mongoose.Error) {
        return res.status(400).json({ message: 'Invalid or missing comment properties' });
      }
      res.status(500).json({ message: 'Something went wrong...' });
    }
  },

  /* User difficulty vote on the exercise */
  postDifficultyVote: async (req, res) => {
    ...
    try {
      ...
      await exercise.save();
      res.status(statusCode).json(exercise);
    } catch (err) {
      console.log(err.message);
      if (err instanceof mongoose.Error.CastError) {
        return res.status(404).json({ message: 'Non existing showcase id' });
